...
st.title("🌌 8D Manifold: 16-Dimensional Jacobian Auditor")

# Standard NASA Physical Constants
...
with st.sidebar:
    ...
    jump_days = st.slider("Analytical Jump (Days)", 1, 365, 365)
    run_audit = st.button("🚀 Execute 16D Jacobian Jump")

if run_audit:
    with st.spinner("Synchronizing 16D Manifold with NASA JPL Horizons..."):
        ...
        for i in range(1, len(BODIES)):
            # Calculate N-Body interaction torsion for the engine
            m_int_sum = 0
            ...
            pert_params = {'M_int': m_int_sum * 10.0} # lambda_opt = 10
            
            # THE 16D JACOBIAN PROPAGATION
            pos_8d = engine.propagate(i, BODIES, r_rel, v_rel, T_jump, pert_params)
        ...
        st.success(f"Audit Complete. Manifold RMSE: {np.sqrt(np.mean(df['Position Error (km)']**2)):,.2f} km.")