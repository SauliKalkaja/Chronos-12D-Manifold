...
class Manifold8DEngine:
    ...
    def propagate(self, planet_idx, bodies, r_input, v_input, T, pert_params, mc_mode=False):
        # 1. Coordinate & Element Extraction
        if mc_mode:
        ...
        # 2. Baseline Keplerian Flow (Time component)
        ...
        # 3. 8D Metric Supervision (Jacobian Scaling)
        ...
        beta_0 = (abs(M_0_manifold) + np.sqrt(M_0_manifold**2 + 4)) / 2.0
        alpha_start = 1.0 / beta_0
        beta_f = (abs(M_f_manifold) + np.sqrt(M_f_manifold**2 + 4)) / 2.0
        alpha_end = 1.0 / beta_f

        # 4. Precession and Frame Alignment
        if n_mag == 0:
        ...
        # Precession is the phase rotation in the manifold
        ...
        # Rotation Matrices to match NASA Inertial Frame
        ...
        # Apply Frame Rotation + Manifold Scaling (alpha_end / alpha_start)
